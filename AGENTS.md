...
### 2026-08-25 — после v0.1.10 (мелкая правка)
- В «Настройках реестра» добавлен заголовок «Группы типов документов» (раздел
  «Сервисный ключ» перенесён в ЦУП — управление всеми сервисными ключами теперь
  центральное, см. sbe-apstore 0.3.6). Пересборка `main.js`/`styles.css`.
- Управление service_secret приложений выполняется централизованно из ЦУП
  (Настройки → Сервисные ключи), серверная часть — auth-service.
