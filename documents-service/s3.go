...
// а SDK внутри HTTP-обработчика зависал и дестабилизировал сервер.
type S3Store struct {
	bucket     string
	configPath string
	publicBase string
...
// s3RemarkKey формирует ключ для файла замечания.
func s3RemarkKey(documentID, fileName string) string {
	return fmt.Sprintf("documents/%s/remarks/%s", documentID, sanitizeKey(fileName))
}
