...
func ownerEmailFromEnv() string {
	return os.Getenv("DOCUMENTS_OWNER_EMAIL")
}
...
SELECT email, role FROM documents_permissions WHERE app = $1 ORDER BY email`, appIDFromEnv())
	if err != nil {
		writeJSON(w, http.StatusInternalServerError, map[string]any{"error": "db error"})
		return
	}
...
DELETE FROM documents_permissions WHERE app = $1 AND email = $2`, appIDFromEnv(), req.Email)
	} else {
		_, err = s.pool.Exec(r.Context(), `
INSERT INTO documents_permissions (app, email, role) VALUES ($1, $2, $3)
...
func (s *Server) handleGetCommonAccess(w http.ResponseWriter, r *http.Request) {
	...
	err := s.pool.QueryRow(r.Context(),
		`SELECT level FROM documents_common_access WHERE app = $1`, appIDFromEnv()).Scan(&level)
	if err != nil {
		level = ""
	}
...
INSERT INTO documents_common_access (app, level) VALUES ($1, $2)
