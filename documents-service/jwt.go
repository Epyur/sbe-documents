...
func appIDFromEnv() string {
	if v := os.Getenv("DOCUMENTS_APP_ID"); v != "" {
		return v
	}
	return "documents"
}
...
func (s *Server) roleFor(ctx context.Context, appID, email string) (string, error) {
	...
	err := s.pool.QueryRow(ctx,
		`SELECT role FROM documents_permissions WHERE app = $1 AND email = $2`,
...
func (s *Server) effectiveRole(ctx context.Context, appID, email string) (string, error) {
	...
	var level string
	err = s.pool.QueryRow(ctx,
		`SELECT level FROM documents_common_access WHERE app = $1`, appID).Scan(&level)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return "", nil
		}
